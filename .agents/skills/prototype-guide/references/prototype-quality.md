...
| **Interaction and State Reachability** | Record Loading, Empty, Error, Partial, and Success as `required|not_applicable`; give every required state a reproducible trigger | A state needed for evaluation cannot be reached, or an omitted state has no decision | Implement the trigger or record why the state cannot affect this validation |
